...
class LatexSlide:
    ...
    def steps(self) -> Iterator[str]:
        return ("\n".join(self._header + step + self._footer) for step in self._steps)
...
def convert_pdf_to_png(file_path_no_ext: Path) -> str:
    command = (
        f'pdftoppm "{file_path_no_ext}.pdf" "{file_path_no_ext}" '
...
def main(_):
    ...
    slide_deck = LatexSlideDeck(read_latex_file(input_path))
